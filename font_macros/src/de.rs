...
use quote::quote;
use syn::{DeriveInput, Fields, Data, LitStr, LitInt, Error};
...
pub fn impl_de_derive(input: &mut DeriveInput) -> TokenStream {
    ...
    match &input.data {
            ...
            match de_fields(&d.fields) {
                Ok(body) => {
                    ...
                    let constructor = match &d.fields {
                        Fields::Named(_) => {
                            let names: Vec<_> = d.fields.iter().map(|field|
                                field.ident.as_ref().unwrap()
                            ).collect();
                            quote!{ #ident { #(#names,)* } }
                        },
                        Fields::Unnamed(_) => {
                            let indices: Vec<_> = (0..d.fields.len()).map(|i| 
                                syn::Index::from(i)
                            ).collect();
                            quote!{ #ident(#(#indices),*) }
                        },
        ...
        _ => {
            Error::new_spanned(input, "derive macro only supports structs").to_compile_error()
        }
    }
}
...
fn de_fields(fields: &Fields) -> syn::Result<Vec<TokenStream>> {
    fields.iter().enumerate().map(|(index, field)| {
        ...
        if !capacity_field.is_empty() {
            let Some(v) = capacity_field.get() else { return Ok(quote!()) };
            return Ok(quote! {
        ...
        if !capacity_with_field.is_empty() {
            let Some(v) = capacity_with_field.get() else { return Ok(quote!()) };
