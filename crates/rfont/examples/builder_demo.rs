...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   压缩率: {:.2}%",
        (subset_data.len() as f64
            / std::fs::metadata("crates/rfont/src/AlimamaDaoLiTi.ttf")?.len() as f64)
            * 100.0
    );
    println!();
    ...
    let subset_data = font.subset_builder().glyph_ids(glyph_ids.clone()).build()?;
